...
def _postgresql_process_checkpoint():
    ...
    try:
        ...
        logger.warning(
            'Your postgresql server is offline,'
            ' fake2db will try to launch it now!', extra=extra_information)
...
def _mongodb_process_checkpoint():
    ...
    try:
        ...
        logger.warning('Your mongodb server is offline,'
                       ' fake2db will try to launch it now!',
...
def main():
    ...
    parser.add_argument("--db", help="Db type for creation: sqlite, mysql, "
                        "postgresql, mongodb, couchdb, to be expanded")
    ...
    if args.rows:
        if args.db:
            ...
            if args.db == 'sqlite':
                ...
                logger.error('Wrong arg for db parameter. Valid ones : '
                             'sqlite - mysql - postgresql - mongodb',
                             extra=extra_information)
        else:
            logger.error('Please use with --help argument for'
                         ' usage information!', extra=extra_information)
